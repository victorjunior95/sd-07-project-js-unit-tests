...
const createMenu = (menu) => {
  const myRestaurant = {
    ...
    consumption: [],
    order(orderString) {
      this.consumption.push(orderString);
    },
    pay() {
      let sumOf = 0;
      const fetchMenu = this.fetchMenu;
      const consumption = this.consumption;
      const foods = Object.keys(fetchMenu.food);
      const drinks = Object.keys(fetchMenu.drink);
      foods.forEach(menuItem => consumption.forEach((orderItem) => {
        if (orderItem === menuItem) sumOf += fetchMenu.food[orderItem];
      }),
      );
      drinks.forEach(menuItem => consumption.forEach((orderItem) => {
        if (orderItem === menuItem) sumOf += fetchMenu.drink[orderItem];
      }),
      );
      return parseFloat(sumOf.toFixed(2));
    },
  };
  return myRestaurant;
};
