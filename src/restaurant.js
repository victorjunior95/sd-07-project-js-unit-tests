...
const createMenu = (menu) => {
  const res = { 
    fetchMenu: menu,
    consumption: [],
    order: (pedido) => res.consumption.push(pedido),
    pay: () => {
      let value = 0;
      res.consumption.forEach((pedido) => {
        Object.keys(res.fetchMenu).forEach((foodDrinks) => {
          if (res.fetchMenu[foodDrinks][pedido] !== undefined) {
            value += res.fetchMenu[foodDrinks][pedido];
          }
        });
      });
