...
/*
  ...
  - Uma chave `fetchMenu` retorna o objeto que a função `createMenu` recebe por parâmetro. O menu tem sempre duas chaves, `food` e `drink`, no
   seguinte formato:
...
// PASSO 3: Crie uma função, separada da função `createMenu()`, que, dada uma string recebida por parâmetro, adiciona essa string ao array de
//`objetoRetornado.consumption`. Adicione essa função à chave `order`.
// DICA: para criar isso, você vai precisar definir a função `createMenu()`, definir o objeto que a `createMenu()` define separadamente dela e,
//depois, a função que será definida em `order`.
// ```
// const restaurant = {}
...
const createMenu = (myMenu) => {
  let menu = {
    ...
    pay: () => {
      let precoTotal = 0;
      let drinkKeys = Object.keys(menu.fetchMenu.drink);
      let foodKeys = Object.keys(menu.fetchMenu.food);
      let drinkValues = Object.values(menu.fetchMenu.drink);
      let foodValues = Object.values(menu.fetchMenu.food);
      ...
      return precoTotal + 0.1 * precoTotal;
    },
  };
  return menu;
};
