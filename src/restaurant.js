...
const createMenu = (inputObject) => {
  const tempObject = {
    fetchMenu: Object.assign({}, inputObject),
    order: element => tempObject.consumption.push(element),
    consumption: [],
    pay: () => {
      let value = 0;
      tempObject.consumption.forEach((element) => {
        Object.keys(tempObject.fetchMenu).forEach((iterator) => {
          if (tempObject.fetchMenu[iterator][element] !== undefined) {
            value += tempObject.fetchMenu[iterator][element];
          }
        });
      });
      return value;
