...
describe('#productDetails', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const checkFinalCode = (obj) => {
      let error = false;
      for (const entrie in obj) {
        if (obj[entrie].details.productId.split("123")[1] !== '') {
          error = true;
        }
      }
      return error;
    }
    assert.deepStrictEqual(checkFinalCode(productDetails('Alcool gel', 'Máscara')), false);
  });
});
