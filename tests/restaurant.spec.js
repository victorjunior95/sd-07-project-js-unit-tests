...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const rec = createMenu({ food: {}, drink: {} });
    const exp = createMenu({ food: {}, drink: {} });
    assert.deepStrictEqual(rec.fetchMenu, exp.fetchMenu);
    ...
    // TESTE 2: Verifique que, dado que a função createMenu foi chamada com o objeto: `{ food: {}, drink: {} }`, verifique que 'objetoRetornado.fetchMenu' retorna um objeto cujas chaves são somente `food` e `drink`.
      ...
      assert.deepStrictEqual(obj.fetchMenu, { food: {}, drink: {} })
    ...
    // objetoRetornado.consumption // Retorno: []
     const objetoMenu = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
     const arrayVazio = [];
     assert.deepStrictEqual(objetoMenu.consumption, arrayVazio);
    ...
    assert.deepStrictEqual(meuRestaurante.consumption, order);
    ...
    const expected = restaurante.consumption;
    assert.deepStrictEqual(restaurante.consumption, expected) // Retorno: ["coxinha", "agua", "sopa", "sashimi"]
    ...
    repetirOrder.order("coxinha");
    const expec = repetirOrder.consumption;
    assert.deepStrictEqual(repetirOrder.consumption, expec);// Retorno: ['coxinha', 'agua', 'coxinha']
    ...
    assert.strictEqual(menu.pay(), "37.95");
