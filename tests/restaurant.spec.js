...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const requireFetchMenu = () => {
      let returMenu = createMenu({ 'coca-cola': 6.99 });
      delete returMenu.consumption;
      delete returMenu.order;
      delete returMenu.pay;
      delete returMenu.resetConsumption;
      return returMenu;
    }
    assert.deepStrictEqual(requireFetchMenu(), { fetchMenu: { 'coca-cola': 6.99 } });
    ...
    const objPassadoParaFuncao = { food: {'Porção': 25.99}, drink: {'Cerveja': 12.99} }
    const newObjetoRetornado = createMenu(objPassadoParaFuncao);
    assert.deepStrictEqual(Object.is(objPassadoParaFuncao, newObjetoRetornado.fetchMenu), true);
    ...
    // ```
    objetoRetornado.resetConsumption();
    objetoRetornado.order('coxinha');
    ...
    meuRestaurante.order('coxinha');
    assert.deepStrictEqual(meuRestaurante.pay(), 11.7);
    
    // meuRestaurante.pay() // Retorno: somaDosPreçosDosPedidos
    // ```
