...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // TESTE 1:
      // Verifique que, dado um objeto qualquer passado como um parâmetro para a função createMenu(),
      // checa se o retorno da função é um objeto no seguinte formato: { fetchMenu: objetoQualquer }.
      assert.deepStrictEqual(Object.entries(objetoRetornado)[0], ['fetchMenu', { food: {}, drink: {} } ]);

    // TESTE 2:
      // Verifique que, dado que a função createMenu foi chamada com o objeto: `{ food: {}, drink: {} }`,
    ...
    // TESTE 3:
      // Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
      assert.deepStrictEqual(objetoRetornado.fetchMenu, { food: {}, drink: {} });

    // TESTE 4:
      // Verifique que 'objetoRetornado.consumption', após a criação do menu, retorna um array vazio.
      assert.deepStrictEqual(objetoRetornado.consumption, []);

    // TESTE 5:
      // Verifique que chamar uma função associada à chave `order` no objeto retornado,
    ...
    // TESTE 6:
      // Verifique que as três orders seguintes, de bebidas e comidas mescladas,
    ...
    // TESTE 7:
      // Verifique que a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.
    ...
    // TESTE 8:
      // Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido,
      // conforme registrado em `objetoRetornado.consumption`
      objetoRetornado = createMenu({ 'agua': 5, 'bolo': 15});
      objetoRetornado.order('agua');
      objetoRetornado.order('bolo');
      assert.deepStrictEqual(objetoRetornado.pay(), 22);
  });
});
