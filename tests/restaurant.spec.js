...
*/
const objetoQualquer = {food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'ceveja': 6.9}};
...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    // TESTE 1: Verifique que, dado um objeto qualquer passado como um parâmetro para a função createMenu(), checa se o retorno da função é um objeto no seguinte formato: { fetchMenu: objetoQualquer }.
      //assert.deepStrictEqual(createMenu({food: {'codinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'ceveja': 6.9}}).fetchMenu, {food: {'codinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'ceveja': 6.9}});
    ...
    objetoRetornado2.order('coxinha');
    assert.deepStrictEqual(objetoRetornado2.consumption, ['coxinha', 'agua', 'coxinha'])
    ...
    const pagamento = createMenu(objetoQualquer);
    //objetoRetornado.order('coxinha');
    //objetoRetornado.order('agua');
    //objetoRetornado.order('coxinha');
    pagamento.order('coxinha');
    pagamento.order('agua');
    pagamento.order('coxinha');
    assert.deepStrictEqual(pagamento.pay(), 11.7);
