...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    assert.deepStrictEqual(createMenu({a:'b'}).fetchMenu(), {a:'b'})
    // ```
    ...
    assert.deepStrictEqual(createMenu({a:'b'}).consumption() , [])
    // ```
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    const objetoRetornado = createMenu({a:'b'});
    ...
    // ```
    const objRetornado2 = createMenu({a:'b'});
    objRetornado2.order("coxinha");
    objRetornado2.order("agua");
    objRetornado2.order("sopa");
    objRetornado2.order("sashimi");
    assert.deepStrictEqual(objRetornado2.consumption(), ["coxinha", "agua", "sopa", "sashimi"])
    ...
    // ```
    const objRetornado3 = createMenu({a:'b'});
    objRetornado3.order('coxinha');
    objRetornado3.order('agua');
    objRetornado3.order('coxinha');
     // Retorno: ['coxinha', 'agua', 'coxinha']
    assert.deepStrictEqual(objRetornado3.consumption(), ['coxinha', 'agua', 'coxinha'])
    // ```
    // Agora faça o TESTE 8 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    // ```
    const objRetornado5 = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    objRetornado5.order('coxinha');
    objRetornado5.order('agua');
    objRetornado5.order('coxinha');
    objRetornado5.pay() // Retorno: somaDosPreçosDosPedidos
    assert.deepStrictEqual(objRetornado5.pay(), 11.7)
    // ```
