...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    assert.ok(createMenu(menu).hasOwnProperty('fetchMenu') && createMenu(menu)['fetchMenu'] === menu);
    // assert.deepStrictEqual(createMenu(menu), resultFetchMenu);
    ...
    // ```
    menu = { food: {}, drink: {} };
    let keysMenu = Object.keys(createMenu(menu).fetchMenu);
    assert.ok(keysMenu.length === 2 && keysMenu[0] === 'food' && keysMenu[1] === 'drink');
    ...
    // ```
    menu = { food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} };
    let resultMenu = createMenu(menu);
    assert.deepStrictEqual(resultMenu.fetchMenu,menu);
    ...
    // ```
    assert.deepStrictEqual(resultMenu.consumption,[]);
    ...
    // ```
    resultMenu.order('coxinha');
    assert.deepStrictEqual(resultMenu.consumption,['coxinha']);
    ...
    // ```
    resultMenu.order('agua');
    resultMenu.order('sopa');
    resultMenu.order('sashimi');
    assert.deepStrictEqual(resultMenu.consumption,['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    // ```
    resultMenu.order('coxinha');
    assert.deepStrictEqual(resultMenu.consumption,['coxinha', 'agua', 'sopa', 'sashimi','coxinha']);
    ...
    // ```
    assert.deepStrictEqual(resultMenu.pay(),23.76);
