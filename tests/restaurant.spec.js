...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const objetoQualquer = { food: {'coxinha': 3.90, 'sanduiche': 9.90},
    drinks: {'agua': 3.90, 'cerveja': 6.90}};
    const result = createMenu(objetoQualquer);
    assert.deepStrictEqual(result.fetchMenu, objetoQualquer);
    // ```
    ...
    const funcaoChaves = { food: {}, drink: {} }
    const testeChaves = createMenu(funcaoChaves)
    const chaves = Object.keys(testeChaves.fetchMenu);
    assert.deepStrictEqual(chaves, ['food', 'drink']);
    // ```
    ...
    assert.deepStrictEqual(result.consumption, ['coxinha']);
    // ```
    // Agora faça o PASSO 3 no arquivo `src/restaurant.js`.
    // --------------------------------------------------------------------------------------
    // TESTE 6: Verifique que as três orders seguintes, de bebidas e comidas mescladas, somam três itens no array `objetoRetornado.consumption` conforme os itens pedidos.
    // ```
    const retorno = createMenu(objetoQualquer);
    retorno.order("coxinha");
    retorno.order("agua");
    retorno.order("sopa");
    retorno.order("sashimi");
    assert.deepStrictEqual(retorno.consumption,["coxinha", "agua", "sopa", "sashimi"])
    ...
    // ```
    const pedidoRepetido = createMenu(objetoQualquer);
    pedidoRepetido.order('coxinha');
    pedidoRepetido.order('agua');
    pedidoRepetido.order('coxinha');
    assert.deepStrictEqual(pedidoRepetido.consumption, ['coxinha', 'agua', 'coxinha'])
    ...
    // ```
    const valor = createMenu(objetoQualquer);
    const somaDosPreçosDosPedidos = 11.7;
    valor.order('coxinha');
    valor.order('agua');
    valor.order('coxinha');
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    assert.strictEqual( valor.pay(), somaDosPreçosDosPedidos)
    // ```
